...
def collect_coin(token, amount):
    ...
    try:
        ...
        print(
            f"HTTP error occurred: {http_err}")  # HTTP-specific error logging
...
def use_special_box(token):
    ...
    try:
        ...
        result = response.json()
        if result['code'] == 0:
            print(f"{Fore.GREEN + Style.BRIGHT}\r[ Chest ] : Activating...",
                  end="",
                  flush=True)
            return True
        else:
            print(f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Failed to activate",
                  end="",
                  flush=True)
            return False
    except Exception as e:
    ...
    try:
        ...
        if result['code'] == 0:
            if result['data']['collectStatus']:
                ...
                print(
                    f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Tidak ada chest          ",
                    flush=True)
                return True, 0
        else:
            print(
                f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Failed to collect coins: {result['message']}",
                end="",
                flush=True)
            return False, 0
    except Exception as e:
...
def attempt_collect_special_box(token, box_type, initial_coin_count):
    ...
    print(
        f"{Fore.RED + Style.BRIGHT}\r[ Chest ] : Unable to collect any coins after adjustments",
        flush=True)
    return 0
...
def recover_coin_pool(token):
    ...
    try:
        ...
        if result['code'] == 0:
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Sukses               ",
                flush=True)
            return True
        else:
            print(f"{Fore.RED + Style.BRIGHT}\r[ Recovery ] : Gagal",
                  flush=True)
            return False
    except Exception as e:
        print(f"{Fore.RED + Style.BRIGHT}\r[ Recovery ] : Error: {e}",
              flush=True)
        return False
    ...
    if tasks:
        for task in tasks:
            ...
            if task['taskStatus'] == 0:  # Jika tugas belum diklaim
                ...
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Task ] : Task already finished          ",
                    flush=True)
...
def main():
    ...
    while True:
        ...
        for index, token in enumerate(tokens):
            ...
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Balance ] : Getting...",
                end="",
                flush=True)
            balance = fetch_account_info(token)
            if balance is None:
                print(f"{Fore.RED}\r[ Balance ] : Failed to get balance",
                      flush=True)
                continue
            else:
                ...
                print(
                    f"{random.choice(available_colors) + Style.BRIGHT}\r[ Balance ] : {balance}           ",
                    flush=True)

            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Game Info ] : Getting...",
                end="",
                flush=True)
            game_info = fetch_account_build_info(token)
            if game_info is None:
                print(f"{Fore.RED}\r[ Game Info ] : Failed to get data",
                      flush=True)
                continue
            else:
                ...
                print(
                    ...
                    flush=True)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Multivalue ] : Level {single_coin_value}",
                    flush=True)
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Coin Limit ] : Level {single_coin_level}",
                    flush=True)
                print(
            ...
            if upgrade_multi_enable == 'y':
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : Upgrading Multi Value...",
                    end="",
                    flush=True)
            ...
            if upgrade_fill_enable == 'y':
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Upgrade ] : Upgrading Fill Rate...",
                    end="",
                    flush=True)
            ...
            if collect_info is None:
                ...
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Collect ] : Collecting Coin...",
                    end="",
                    flush=True)
                if coin_pool_left_count > 0:
                    ...
                    if collect_result and collect_result.get('code') == 0:
                        collected_amount = collect_result['data'][
                            'collectAmount']
                        print(
                        ...
                        print(
                            f"{random.choice(available_colors)+Style.BRIGHT}\r[ Collect ] : Failed to collect coins",
                            flush=True)

            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : Trying to activate...",
                end="",
                flush=True)
            if game_info and game_info['data'].get(
                    'specialBoxLeftRecoveryCount', 0) > 0:
                if use_special_box(token):
                    print(
                        f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : Collecting...",
                        end="",
                        flush=True)
                ...
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Chest ] : No chest available          ",
                    flush=True)
            time.sleep(1)
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : Trying to recover...",
                end="",
                flush=True)
            game_info = fetch_account_build_info(token)
            if game_info and game_info['data'].get('coinPoolLeftRecoveryCount',
                                                   0) > 0:
                if recover_coin_pool(token):
                    ...
                    if collect_info:
                        coin_pool_left_count = collect_info['data'].get(
                            'coinPoolLeftCount', 0)
                        if coin_pool_left_count > 0:
                            amount = coin_pool_left_count // game_info[
                                'data'].get('singleCoinValue', 1)
                            collect_result = collect_coin(token, amount)
                            if collect_result and collect_result.get(
                                    'code') == 0:
                                collected_amount = collect_result['data'][
                                    'collectAmount']
                                print(
                                    f"{Fore.GREEN + Style.BRIGHT}\r[ Collect ] : Berhasil mengumpulkan {collected_amount} koin",
                                    flush=True)
                            else:
                                print(
                                    f"{Fore.RED + Style.BRIGHT}\r[ Collect ] : Gagal mengumpulkan koin",
                                    flush=True)
            else:
                print(
                    f"{random.choice(available_colors)+Style.BRIGHT}\r[ Recovery ] : No recovery available",
                    flush=True)
            time.sleep(1)
            print(
                f"{random.choice(available_colors)+Style.BRIGHT}\r[ Free Chest ] : Trying to collect..",
                end="",
                flush=True)
        ...
        print(
            ...
            flush=True)
        import sys
        waktu_tunggu = 240  # 5 menit dalam detik
...
def parse_arguments():
    ...
    parser.add_argument('--task',
                        type=str,
                        choices=['y', 'n'],
                        help='Cek and Claim Task (y/n)')
    parser.add_argument('--multi',
                        type=str,
                        choices=['y', 'n'],
                        help='Upgrade Multi Value (y/n)')
    parser.add_argument('--fill',
                        type=str,
                        choices=['y', 'n'],
                        help='Upgrade Fill Rate (y/n)')
    ...
    #     task_input = input(
    #         "Apakah Anda ingin cek dan claim task? (y/n, default n): ").strip(
    #         ).lower()
    #     args.task = task_input if task_input in ['y', 'n'] else 'n'

    # if args.multi is None:
    #     multi_input = input(
    #         "Apakah Anda ingin upgrade multi value? (y/n, default n): ").strip(
    #         ).lower()
    #     args.multi = multi_input if multi_input in ['y', 'n'] else 'n'

    # if args.fill is None:
    #     fill_input = input(
    #         "Apakah Anda ingin upgrade fill rate? (y/n, default n): ").strip(
    #         ).lower()
